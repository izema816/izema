    ...
    if response == "1":
        ...
        if ip_addr in scanner.all_hosts():
         for proto in scanner[ip_addr].all_protocols():
            print("\nprotocol: "+ proto )
            for ports in scanner[ip_addr][proto].keys():
                state = scanner[ip_addr][proto][ports]["state"]
                name = scanner[ip_addr][proto][ports]["name"]
                ver = scanner[ip_addr][proto][ports].get("version", "")
            print("ports: " + str(ports) + "|" + state + "|" + name + "|" + ver )
    elif response == "4":
        print("shutting down.......")
        break
    else:
